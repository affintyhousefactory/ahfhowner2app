...
- **Pages éditoriales — le registre est la seule source de routes** (ADR-038). Les 19 pages du chantier « PagesSite_SEO » (usages, guides, local) se déclarent dans `src/lib/pages/registry.ts` ; sitemap, navigation, fil d'Ariane et maillage en dérivent. **Ne jamais écrire une route de ce chantier ailleurs.** Une page ne passe de `"a-venir"` à `"publiee"` qu'une fois servie et vérifiée en Preview — le sitemap ne doit annoncer que ce qui existe.
...
> **Exception unique (ADR-029 § Amendement du 2026-08-20)** : « tiny house » est autorisé sur la seule page `/studio-jardin-tiny-house`, où il désigne le produit concurrent qu'on écarte — jamais un Arko. Portée par le champ `sauf` de `check-vocabulaire.mjs`, qui lève **un terme sur un chemin** sans désarmer le reste du contrôle. `sauf` ne s'emploie que pour une exception écrite dans un ADR.
...
> **Doctrine lexicale (ADR-029 § Amendement du 2026-09-11, `docs/specs/DOCTRINE_LEXICALE_HOWNER.md`)** : le mot dit le **rôle réellement exercé**. Howner est **fabricant-installateur** (fabrication **hors-site**, atelier, livraison, installation, pose) — jamais constructeur, ni maître d'œuvre, ni cabinet d'architecture ; ni « permis garanti », « validation urbanistique », « nous nous occupons de tout », « projet entièrement suivi par notre architecte » (tous dans `PROSCRITS`). Avec prudence : architecture, conception, étude de faisabilité, implantation, accompagnement — légitimes pour décrire le studio, fautifs pour qualifier Howner. « architecte » : seulement pour une personne habilitée et désignée — **point ouvert n° 4 d'ADR-029, arbitrage de Richard**.
