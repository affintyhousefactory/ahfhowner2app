...
- **CRM (ADR-035)** : les 8 statuts commerciaux, les conseillers, le seuil de silence et les pièces du dossier vivent dans `src/lib/crm.ts` — **jamais redéclarés dans un écran**. Les libellés de configuration se résolvent par `loadConfig()`, jamais stockés sur le lead. `responsable` ≠ `mandataire_id` : ne pas confondre le conseiller AHF avec le domaine suspendu.
