...
## Résumé exécutif
Site **bi-produit** de réservation — **Arko One** (20 m², 12 exemplaires) + **Arko Max** (40 m², 5 exemplaires), ADR-022. **Front complet et validé** (Lighthouse 100/100/100/100, LCP 0.8s, CLS 0). **Portails admin et mandataire livrés et en production.** **Lancement commercial bloqué par le légal.** ⚠️ **Voir « Dernier point » 2026-07-13 — CGV non confirmées avocat déjà live en prod, liées au tunnel de réservation.** Sources de vérité : `src/lib/site.ts` (marque/pricing), `DESIGN.md` (charte), `03_DECISIONS/` (ADR).
...
12. **Arko Max pricing grid** (perM2/options/terrasse/footprint/`reserved`) — toujours en attente données métier + grille tarifaire.
13. **Coordonnées atelier** : placeholder Bayonne (43.4933 / −1.4748) — à affiner.
